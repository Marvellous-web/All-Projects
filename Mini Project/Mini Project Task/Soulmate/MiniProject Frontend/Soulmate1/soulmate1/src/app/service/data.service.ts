...
export class DataService {
  ...
  getFormNeo():Observable<Customer[]> {
    return this.http.get<Array<Customer>>('http://localhost:8084/person/postfind');
  }
  ...
  getAllUser() {
    ...
    if (key) {
      const decode: JWT.JwtPayload = jwtDecode(key);
      this.http.get<Array<Customer>>('http://localhost:8081/soul-mate/getUser').subscribe((value) => {
